...
import { getApiBaseUrl, getStaticBaseUrl } from '../config/api.config';
...
export class AuthService {
  ...
  private apiUrl = `${getApiBaseUrl()}/api/v1/auth`;
  ...
  cachedAvatarUrl = computed(() => {
    ...
    if (user && user.avatar_url) {
      return `${getStaticBaseUrl()}${user.avatar_url}?v=${Date.now()}`;
    }
    return null;
  });
