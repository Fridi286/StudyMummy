...
@Component({
  selector: 'app-shop',
  standalone: true,
  imports: [CommonModule, FormsModule, CardModule, ButtonModule, ToastModule, ProgressSpinnerModule, InputNumberModule, TabsModule, AvatarUrlPipe],
...
export class ShopComponent implements OnInit {
  ...
  loadShop() {
    ...
    // Run both requests in parallel; inventory failure returns null (non-blocking)
    forkJoin({
      inventory: this.inventoryService.getInventory().pipe(catchError(() => of(null))),
      items: this.shopService.getShopItems()
    }).subscribe({
      next: ({ inventory, items }) => {
        // Build owned set (gracefully handle null inventory)
        const owned = new Set<string>();
        if (inventory) {
          inventory.inventory.forEach(i => owned.add(i.item.item_id));
          inventory.active_items.forEach(a => owned.add(a.item.item_id));
        }
        ...
        this.shopCategories.set(Object.keys(groups).map(k => ({ type: k, items: groups[k] })));
